...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _btnView.frame = CGRectMake(0,0,self.frame.size.width, 30);
        [self createScroll];
        [self CreatelineView];
...
- (void)createScroll {
    ...
    _pageScroll.frame = CGRectMake(0,30, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height -64);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (_seletedBtn != btn) {
        [UIView animateWithDuration:0.1 animations:^{
            ...
            self.lineView.frame = CGRectMake(x, 28, width,2);
        }];
        _seletedBtn = btn;
    }
}
